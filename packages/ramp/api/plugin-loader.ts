...
export default class Loader {
    ...
    destroyer() {
        this.plugins
            .concat(Object.keys(this.features).map(key => this.features[key]))
    ...
    loader() {
        ...
        Object.keys(AUTOLOAD_PLUGINS).forEach(pk => {
            ...
            if (!pN && loadFeatures[pk]) {
                console.warn(
                    `The plugin ${pk} was loaded automatically. This functionality is being removed in the next major release. Please load this plugin on the host page instead (see ramp documentation) or add 'no-${pk}' to rv-plugins to stop this plugin from being autoloaded.`
                );
            } else if (pN === `no-${pk}`) {
        ...
        Object.keys(loadFeatures).forEach((k: string) => {
     ...
     */
    preInit() {
        this.plugins.concat(Object.values(this.features)).forEach(p => {
            if (p.preInit) {
